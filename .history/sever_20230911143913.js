...
let dbSecretKey = '-----BEGIN RSA PRIVATE KEY-----

6JUbHZ0uUr/0AYqUKmRrgs1Y6qpoU29uudAFYn1F
-----END RSA PRIVATE KEY-----';

resDecrypt = (text, key) => {
    const privateKey = forge.pki.privateKeyFromPem(key);
    const decryptedData = privateKey.decrypt(forge.util.decode64(text), 'RSA-OAEP');
...
app.get('/GioHang',(req, res) => {
    ...
    maNguoiDung = maNguoiDung.replace(/ /g, '+');
    console.log(dbSecretKey);
    // console.log('Code ma hóa         ',resDecrypt(maNguoiDung, dbSecretKey));
    async function fetchDataCustomers(){
        try {
            const connection = await oracledb.getConnection({
                user: 'hr',
