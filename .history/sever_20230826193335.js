...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const query = 'SELECT * FROM hr.Login WHERE TenDangNhap = :username';
        const result = await connection.execute(query, [username]);
        console.log(result);
        if (result.rows.length === 0) {
            res.status(402).json({ success: false, message: 'Tên đăng nhập không tồn tại' });
            return;
        }

        const storedHashedPassword = result.rows[0].MATKHAU;
